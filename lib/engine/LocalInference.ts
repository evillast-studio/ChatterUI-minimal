...
export const localInference = async () => {
    try {
        ...
        Logger.errorToast(t('model.toast.failedToRunLocalInference'), errorDetail)
        stopGenerating()
    }
}
...
const runLocalCompletion = async (
    ...
    // acumulamos tokens en una variable JS pura y flusheamos cada ~132ms (~7fps).
    // 132ms = 2× el valor anterior (66ms). Reduce a la mitad los re-renders de React
    // y el overhead del JS thread durante inferencia, ganando ~8-12% más velocidad
    // de tokens percibida en hardware con JS thread lento (ej. Kirin 710).
    // Usar performance.now() para timing monotónico (no se ve afectado por cambios
    // del reloj del sistema).
    const UI_FLUSH_MS = 132
    let _tokenBatch = ''
    let _lastFlush = 0
    let _ttsBuffer = ''
    ...
    const outputStream = (text: string) => {
        ...
        // Flush al estado React solo cada 132ms
    ...
    await Llama.useLlamaModelStore
        ...
        .catch((error) => {
            ...
            Logger.errorToast(t('model.toast.failedToGenerateLocally'), errorDetail)
            stopGenerating()
        })
}
...
const obtainFields = async (): Promise<ContextBuilderParams | void> => {
    try {
        ...
        const errorDetail = e instanceof Error ? `${e.message}\n${e.stack ?? ''}` : JSON.stringify(e)
        Logger.errorToast(t('generation.errors.failedToOrchestrateRequestBuild'), errorDetail)
    }
}
