    ...
    export const createDefaultCard = async () => {
        ...
        try {
            if (!fileExists(cardDefaultDir)) {
                ...
                if (!asset.localUri) throw new Error('Failed to load default card asset')
                // await: si no se espera, createCharacterFromImage corre antes de que exista el archivo (ENOENT)
