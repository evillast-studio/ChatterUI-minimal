...
export type LlamaConfig = {
    ...
    n_keep: number
    defrag_thold: number
    // Memory pruning (poda automatica de mensajes viejos, reemplaza al ctx_shift lento)
    prune_enabled: boolean
    prune_soft_percent: number
    prune_hard_percent: number
    // Force backend
...
const defaultConfig: LlamaConfig = {
    ...
    defrag_thold: 0.4,
    prune_enabled: true,
    prune_soft_percent: 75,
    prune_hard_percent: 90,
    // Force backend
    force_device: false,
    custom_device: '',
        ...
        persist(
            ...
            }),
            {
                ...
                migrate: (persistedState: any, version) => {
                    ...
                    if (version === 7) {
                        // v8: ajustes de Memory Pruning
                        persistedState.config.prune_enabled = true
                        persistedState.config.prune_soft_percent = 75
                        persistedState.config.prune_hard_percent = 90
                        Logger.info('Migrated to v8 EngineData: memory pruning settings')
                    }
