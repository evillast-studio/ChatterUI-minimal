...
const ModelSettings: React.FC<ModelSettingsProp> = ({ modelImporting, modelLoading, exit }) => {
    ...
    const applyPreset = (partialConfig: Partial<LlamaConfig>) => {
        // Ignora claves undefined (presets guardados antes de existir ajustes nuevos)
        const clean = Object.fromEntries(
            Object.entries(partialConfig).filter(([, v]) => v !== undefined)
        ) as Partial<LlamaConfig>
    ...
    // Respaldo por si la config guardada es anterior a Memory Pruning
    const pruneEnabled = config.prune_enabled ?? true
    const pruneSoft = config.prune_soft_percent ?? 75
    const pruneHard = config.prune_hard_percent ?? 90
    ...
    return (
        <Animated.ScrollView
            ...
            {/* ── Memory Pruning (alternativa rápida al Context Shift) ── */}
            <SectionTitle>Memory Pruning</SectionTitle>
            <View style={{ marginTop: 8 }} />
            <ThemedSwitch
                label="Memory Pruning automático"
                value={pruneEnabled}
                description="Descarta los mensajes más antiguos antes de que el contexto se llene. Evita el Context Shift lento. Funciona con cualquier tamaño de contexto."
                onChangeValue={(v) => setConfig({ ...config, prune_enabled: v })} />
            {pruneEnabled && (
                <>
                    <ThemedSlider
                        label="Poda suave (%)"
                        ...
                        onValueChange={(v) =>
                            setConfig({
                                ...config,
                                prune_soft_percent: v,
                                prune_hard_percent: Math.max(pruneHard, v + 3),
                            })
                        }
                    ...
                    <Text style={{ color: '#aaa', fontSize: 12, marginHorizontal: 4, marginBottom: 12 }}>
                        Al llegar a {pruneSoft}% del contexto se descartan 1-2 mensajes antiguos.
                    </Text>
                    <ThemedSlider
                        label="Poda dura (%)"
                        value={pruneHard}
                        onValueChange={(v) =>
                            setConfig({
                                ...config,
                                prune_hard_percent: v,
                                prune_soft_percent: Math.min(pruneSoft, v - 3),
                            })
                        }
                        min={40} max={98} step={1}
                        disabled={modelImporting || modelLoading} />
                    <Text style={{ color: '#aaa', fontSize: 12, marginHorizontal: 4, marginBottom: 12 }}>
                        Al llegar a {pruneHard}% se descartan más mensajes de golpe. Debe ser mayor que la poda suave.
                    </Text>
                </>
            )}
